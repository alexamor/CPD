...
int main(int argc, char *argv[]){
	...
	int length_of_error_string, error_code, namelen;
    ...
    */
    ...
    for(t = 0; t < n_tstep; t++){
		...
		MPI_Allreduce(x_cell, x_cell_sub, ncside * ncside, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		//MPI_Barrier(MPI_COMM_WORLD);   


		MPI_Allreduce(y_cell, y_cell_sub, ncside * ncside, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		//MPI_Barrier(MPI_COMM_WORLD);

		MPI_Allreduce(m_cell, m_cell_sub, ncside * ncside, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		//MPI_Barrier(MPI_COMM_WORLD);
